            ...
            <div className="p-2 flex w-full justify-between">
                <div className="flex">
                    <IconComment width={28} height={28}/>
                        <span className=" ml-2 text-lg">{permissions.includes(EPostPermission.ALLOW_COMMENT) ? comments.length : "N/A" }</span>
                </div>
                <button className="flex">
                    <IconHeart width={28} height={28} fill="red"/>
                    <span className=" ml-2 text-lg">5</span>
                </button>
                <button className={`flex ${permissions.includes(EPostPermission.ALLOW_SHARE) ? 'text-green-500' : 'text-gray-400'}`} disabled={!permissions.includes(EPostPermission.ALLOW_SHARE)}>Share</button>
            </div>
            </div>
            <div>
                {
